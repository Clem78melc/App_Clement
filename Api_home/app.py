...
explainer = LimeTabularExplainer(pipeline[0].transform(test[features]), 
...
def predict():
    ...
    # Perform prediction using your home credit model
    threshold = 0.49756
    prediction_proba = pipeline.predict_proba(data)[:, 1]  # Assuming binary classification
    prediction = (prediction_proba >= threshold).astype(int)

    # Generate local feature importance with LIME
    transform_data = pipeline[0].transform(data)
    lime_explanation = explainer.explain_instance(transform_data[0], pipeline[1].predict_proba)
    ...
    gauge = 'Credit Accepted' if prediction == 0 else 'Credit Not Accepted'
